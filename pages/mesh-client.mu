#!bg=020617
#!fg=d1d5db
`c
`FT86efac`!THE COLORADO MESH MESH CLIENT`!`f
`FT334155
-
`f
`a
`:top

Instead of juggling three separate apps, get one desktop client
for the three major mesh ecosystems:

`tc
Protocol | Role
--- | ---
`FT86efacMeshtastic`f | LoRa mesh
`FT22d3eeMeshCore`f | Companion radio
`FTf59e0bReticulum`f | Nomad host
`t

The Colorado Mesh Mesh Client is a cross-platform Electron app
for macOS, Linux, and Windows. Connect over BLE, USB serial,
or Wi-Fi/TCP and keep local message history in SQLite.

`FT334155
-.
`f
`FT94a3b8On this page:`f
`FT60a5fa`[Jump to Get started`#get-started]`f
`FT60a5fa`[Jump to Highlights`#highlights]`f
`FT60a5fa`[Jump to Open source`#open-source]`f
`FT334155
-.
`f

`:get-started`FT60a5fa`!GET STARTED`!`f
`FT334155
-.
`f
  * Download the installer for your platform
  * Install and open the app
  * Connect a radio over BLE, serial, or Wi-Fi

`BT1e293b`FTfbbf24`*`[ Download latest release`https://github.com/Colorado-Mesh/mesh-client/releases]`*`b`f

`:highlights`FT60a5fa`!HIGHLIGHTS`!`f
`FT334155
-.
`f
  * Messaging across supported mesh protocols
  * Multi-language interface (16 languages)
  * MQTT connectivity
  * Routing diagnostics and topology tools
  * Local-first message history in SQLite
  * `FTf59e0bNomad Network`f browsing and page hosting
  * TAK server for ATAK/WinTAK/iTAK
  * Interactive map with position trails and waypoint overlays
  * Node health scores and telemetry charts

This very page is served by its built-in Reticulum
Nomad Network host.

`:open-source`FT60a5fa`!OPEN SOURCE`!`f
`FT334155
-.
`f

`BT1e293b`FTfbbf24`*`[ Project website`https://colorado-mesh.github.io/mesh-client/]`*`b`f
`FT86efac`*`[ Source code on GitHub`https://github.com/Colorado-Mesh/mesh-client]`*`f

`:related-projects`FT60a5fa`!RELATED PROJECTS`!`f
`FT334155
-.
`f
`FTf59e0b`*`[NomadNet`https://github.com/markqvist/NomadNet]`*`f
Decentralized information network over Reticulum - this very
page is hosted on one.

`FTf59e0b`*`[Reticulum`https://reticulum.network/]`*`f
The encrypted networking stack that powers Nomad Network.

`FT22d3ee`*`[Sideband`https://github.com/markqvist/Sideband]`*`f
Mobile messaging app for LXMF conversations over Reticulum.

`FT86efac`*`[Colorado Mesh`https://coloradomesh.org]`*`f
The community this client is built for.

`FT334155
-.
`f
`FT94a3b8`[ Home`:/page/index.mu]`f
`FT86efac`[ About`:/page/about.mu]`f
`FTf59e0b`[ Community`:/page/community.mu]`f
`FT60a5fa`[ Resources`:/page/resources.mu]`f
`FTf59e0b`[ Links`:/page/links.mu]`f
`FT94a3b8`[ Back to top`#top]`f
